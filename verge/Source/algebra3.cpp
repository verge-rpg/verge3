/*
  ...
  **************
  ...
  in minor ways in order to compile more modernly.
...
#ifdef _MSC_VER
//disable warning for empty controlled statements 
...
vec2 &vec2::operator/=(A3_TYPE d)
{ 
    A3_TYPE d_inv = 1.0f/d; 
    n[VX] *= d_inv; 
    n[VY] *= d_inv; 
    return *this; 
}

A3_TYPE &vec2::operator[](int i) 
{
    if (i < VX || i > VY)
...
const A3_TYPE &vec2::operator[](int i) const
{
    if (i < VX || i > VY)
      //VEC_ERROR("vec2 [] operator: illegal access; index = " << i << '\n')
      VEC_ERROR("vec2 [] operator: illegal access" );
...
A3_TYPE vec2::length() const 
{ 
    return (A3_TYPE) sqrt(length2()); 
}

A3_TYPE vec2::length2() const 
{ 
...
void vec2::set( A3_TYPE x, A3_TYPE y )
{
  n[VX] = x;   n[VY] = y; 
}
...
vec2 operator*(const vec2 &a, A3_TYPE d)
{ 
    return vec2(d*a.n[VX], d*a.n[VY]); 
}

vec2 operator*(A3_TYPE d, const vec2 &a)
{ 
    return a*d; 
}
...
A3_TYPE operator*(const vec2 &a, const vec2 &b)
{ 
...
vec3::vec3(A3_TYPE x, A3_TYPE y, A3_TYPE z)
{ 
    n[VX] = x; 
    n[VY] = y; 
    n[VZ] = z; 
...
vec3::vec3(const vec2 &v, A3_TYPE d)
{ 
    n[VX] = v.n[VX]; 
    n[VY] = v.n[VY]; 
    n[VZ] = d; 
...
const A3_TYPE &vec3::operator[](int i) const
{
    if (i < VX || i > VZ)
        //VEC_ERROR("vec3 [] operator: illegal access; index = " << i << '\n')
        VEC_ERROR("vec3 [] operator: illegal access" );
...
A3_TYPE vec3::length() const
{  
    return (A3_TYPE) sqrt(length2()); 
}

A3_TYPE vec3::length2() const
{  
...
void vec3::set(A3_TYPE x, A3_TYPE y, A3_TYPE z)   // set vector
{ 
    n[VX] = x; 
    n[VY] = y; 
    n[VZ] = z;  
...
vec3 operator*(const vec3 &a, A3_TYPE d)
{ 
    return vec3(d*a.n[VX], d*a.n[VY], d*a.n[VZ]); 
}

vec3 operator*(A3_TYPE d, const vec3 &a)
{ 
    return a*d; 
}
...
A3_TYPE operator*(const vec3 &a, const vec3 &b)
{ 
...
vec4::vec4(A3_TYPE x, A3_TYPE y, A3_TYPE z, A3_TYPE w)
{ 
    n[VX] = x; 
    n[VY] = y; 
    n[VZ] = z; 
...
vec4::vec4(const vec3 &v, A3_TYPE d)
{ 
    n[VX] = v.n[VX]; 
    n[VY] = v.n[VY]; 
    n[VZ] = v.n[VZ];  
...
const A3_TYPE &vec4::operator[](int i) const
{
    if (i < VX || i > VW)
        //VEC_ERROR("vec4 [] operator: illegal access; index = " << i << '\n')
        VEC_ERROR("vec4 [] operator: illegal access" );
...
A3_TYPE vec4::length() const
{ 
    return (A3_TYPE) sqrt(length2()); 
}

A3_TYPE vec4::length2() const
{ 
...
void vec4::set(A3_TYPE x, A3_TYPE y, A3_TYPE z, A3_TYPE a)
{
    n[0] = x; 
    n[1] = y; 
    n[2] = z; 
...
vec4 operator*(const vec4 &a, A3_TYPE d)
{ 
    return vec4(d*a.n[VX], d*a.n[VY], d*a.n[VZ], d*a.n[VW]); 
}

vec4 operator*(A3_TYPE d, const vec4 &a)
{ 
    return a*d; 
}
...
A3_TYPE operator*(const vec4 &a, const vec4 &b)
{ 
    return 
        a.n[VX]*b.n[VX] + 
        a.n[VY]*b.n[VY] + 
...
mat3 operator*(const mat3 &a, A3_TYPE d)
{ 
    return mat3(a.v[0]*d, a.v[1]*d, a.v[2]*d); 
}

mat3 operator*(A3_TYPE d, const mat3 &a)
{ 
    return a*d; 
}
...
mat4::mat4(
     A3_TYPE a00, A3_TYPE a01, A3_TYPE a02, A3_TYPE a03,
...
mat4 operator*(const mat4 &a, A3_TYPE d)
{ 
    return mat4(a.v[0]*d, a.v[1]*d, a.v[2]*d, a.v[3]*d); 
}

mat4 operator*(A3_TYPE d, const mat4 &a)
{ 
    return a*d; 
}
...
mat3 rotation2D(const vec2 &Center, A3_TYPE angleDeg) 
{
    A3_TYPE angleRad = (A3_TYPE) (angleDeg * M_PI / 180.0);
    A3_TYPE c = (A3_TYPE) cos(angleRad);
    A3_TYPE s = (A3_TYPE) sin(angleRad);
...
mat4 rotation3D(const vec3 &Axis, A3_TYPE angleDeg) 
{
    A3_TYPE angleRad = (A3_TYPE) (angleDeg * M_PI / 180.0);
    A3_TYPE c = (A3_TYPE) cos(angleRad);
    A3_TYPE s = (A3_TYPE) sin(angleRad);
...
mat4 rotation3Drad(const vec3 &Axis, A3_TYPE angleRad) 
{
    A3_TYPE c = (A3_TYPE) cos(angleRad);
    A3_TYPE s = (A3_TYPE) sin(angleRad);
    A3_TYPE t = 1.0f - c;
