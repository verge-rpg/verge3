...
- (void)fixMenu:(NSMenu *)aMenu withAppName:(NSString *)appName
{
    ...
    // we replace "verge3", which is what is used by the .nib file's menus
    aRange = [[aMenu title] rangeOfString:@"verge3"];
    if (aRange.length != 0)
    ...
    {
        aRange = [[menuItem title] rangeOfString:@"verge3"];
        if (aRange.length != 0)
...
#if SDL_USE_NIB_FILE
    /* Set the main menu to contain the real app name instead of "verge3"
        uses bundle name, which is what is used for the app menu */
     NSString *name = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleName"];
     if(name == nil) {
        name = @"verge3"; // default
     }
     [self fixMenu:[NSApp mainMenu] withAppName:name];
