...
template <class T> using has_insertion_operator = has_insertion_operator_impl::check<const T>;
...
template <typename T> struct StringMaker : public detail::StringMakerBase<detail::has_insertion_operator<T>::value>
{
};
...
struct ConsoleReporter : public IReporter
{
    ...
    void test_run_end(const TestRunStats& p) override
    {
        ...
        const auto passwidth = int(std::ceil(log10((std::max(p.numTestCasesPassingFilters - p.numTestCasesFailed,
                                                             static_cast<unsigned>(p.numAsserts - p.numAssertsFailed)))
                                                   + 1)));
        const auto failwidth
...
bool parseIntOption(int argc, const char* const* argv, const char* pattern, optionType type, int& res)
{
    ...
    else
    {
        ...
        // failed parse...
        const int theInt = std::atoi(parsedValue.c_str()); // NOLINT
        if (theInt != 0)
        {
...
int Context::run()
{
    ...
    if (!testArray.empty())
    {
        ...
        else if (p->order_by.compare("rand", true) == 0)
        {
            ...
            {
                const int idxToSwap = std::rand() % (i + 1); // NOLINT
